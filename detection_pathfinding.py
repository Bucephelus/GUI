...
def detect_maze_obstacles(image_path):
    ...
    _, thresh_walls = cv2.threshold(gray_walls, 40, 255, cv2.THRESH_BINARY)
...
def detect_green_points(image):
    ...
    lower_green = np.array([85, 230, 119])
    upper_green = np.array([91, 255, 150])
    ...
    for cnt in contours:
        # Calculate the contour area and ignore small areas
...
# Assume frame_list is already defined and contains the coordinates of the frame's vertices
# For example: frame_list = [(x1, y1), (x2, y2), (x3, y3), (x4, y4)]
...
for y in range(grid_size // 2, height, grid_size):
    for x in range(grid_size // 2, width, grid_size):
        if point_inside_polygon(x, y, frame_list):
...
grid_coords = remove_close_coords(grid_coords, wall_list, d1)
grid_coords = remove_close_coords(grid_coords, frame_list, d2)
...
for coord in grid_coords:
    ...
    for other_coord in grid_coords:
        if coord != other_coord:
            ...
            if dist < 25:  # Adjust the threshold as needed 
...
# STORES NECESSARY COORDINATES OF PATH:
